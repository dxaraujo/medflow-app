...
import { fetchConsultRecord } from "@/lib/api"
...
export function PacienteConsultaDetalhePage() {
  ...
  const [record, setRecord] = useState<PatientConsultRecord | null | undefined>(undefined)

  useEffect(() => {
    if (!patientId || !consultId) return
    let cancelled = false
    fetchConsultRecord(patientId, consultId).then((r) => {
  ...
  if (record === undefined) {
    return (
      <div className="col-span-12 py-12 text-center font-body text-on-surface-variant">Carregando consulta…</div>
    )
  }
