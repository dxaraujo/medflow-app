...
function PacienteDetalheRoute() {
  const { patientId } = useParams<{ patientId: string }>()
  return <PacienteDetalheLayout key={patientId} />
}

function ConsultaDetalheRoute() {
  const { patientId, consultId } = useParams<{ patientId: string; consultId: string }>()
  return <PacienteConsultaDetalhePage key={`${patientId}-${consultId}`} />
}
    ...
    <BrowserRouter>
      ...
      <main className="ml-64 min-h-screen">
        ...
        <Routes>
          ...
          <Route path="/pacientes/:patientId" element={<PacienteDetalheRoute />}>
            ...
            <Route path="historico/consultas/:consultId" element={<ConsultaDetalheRoute />} />
